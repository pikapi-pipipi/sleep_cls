...
class EOGBackbone(nn.Module):
    
    def __init__(self, config, kernel_size=3):
        super(EOGBackbone, self).__init__()
...
class EOGEncoder(nn.Module):
    def __init__(self, config):
        super(EOGEncoder, self).__init__()
        self.cfg = config
        self.backbone = EOGBackbone(config)
        self.SeqAttn = AgentTransformer(config, 8, 4, pool_size_rate=1, 
                                        seq_len=config['Transformer']['eog_pos_enc']['seq_len'],
                                        pos_enc_dropout=config['Transformer']['eog_pos_enc']['dropout'])
